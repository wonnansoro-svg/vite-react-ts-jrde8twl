...
const getWeatherVisuals = (code: number) => {
  if (code === 0 || code === 1) return { 
    ...
    actionType: "harvest", 
    ttsText: "Ciel dégagé et très ensoleillé. Suggestion : C'est le moment idéal pour récolter l'anacarde ou désherber manuellement." 
  };
  ...
  if (code >= 80) return { 
    ...
    actionImg: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQiHyDydvnCDwg_HZHcnOlBqQrXb5TePETSAQ&s", 
    actionType: "spray_no", 
    ttsText: "Alerte orages violents. Restez à l'abri. Ne vous rendez pas dans les champs sous les éclairs." 
  };
  return { 
    weatherImg: "https://images.unsplash.com/photo-1485236715568-ddc5ee6ca227?w=400", 
    Icon: Cloud, 
    actionImg: "https://img.freepik.com/photos-gratuite/gros-plan-photo-main-tenant-plantation-graine-plante_1150-28369.jpg", 
    actionType: "sowing", 
    ttsText: "Temps clément. Vous pouvez continuer l'observation de vos cultures et le sarclage." 
  };
};
    ...
    <div className="flex flex-col h-full bg-gray-50 overflow-y-auto relative pb-20">
      ...
      <div className="relative h-[45%] min-h-[320px] flex-shrink-0 border-b-4 border-green-600 rounded-b-3xl shadow-md overflow-hidden z-0 bg-gray-200">
        <MapContainer center={[location.lat, location.lon]} zoom={14} style={{ height: '100%', width: '100%', zIndex: 0 }} zoomControl={false}>
          ...
          <Polygon positions={polyAnacarde} pathOptions={{ color: '#ea580c', fillColor: '#ea580c', fillOpacity: 0.5 }}><Popup>Parcelle 2 : Anacarde</Popup></Polygon>
        </MapContainer>
      </div>
      ...
      <div className="p-4 pt-6 space-y-6">
        <div>
          ...
          <div className="flex overflow-x-auto space-x-4 pb-2 -mx-4 px-4 scrollbar-hide">
            ...
            <div onClick={() => setSelectedCrop('Anacarde')} className="cursor-pointer bg-white rounded-2xl shadow-sm min-w-[220px] flex-shrink-0 border border-gray-100 overflow-hidden"><div className="relative h-28"><img src="https://images.unsplash.com/photo-1532296068339-4ab529dc3709?w=400" alt="Anacarde" className="w-full h-full object-cover" /></div><div className="p-4"><h3 className="font-bold text-gray-800">Parcelle 2 - Anacarde</h3></div></div>
          </div>
        </div>
      </div>
      
      {selectedCrop && (
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/60"><div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-2xl relative"><h3 className="text-xl font-black mb-3">Détails : {selectedCrop}</h3><p className="text-gray-600 mb-4">Analyse de votre parcelle de {selectedCrop.toLowerCase()}...</p><button onClick={() => setSelectedCrop(null)} className="w-full bg-green-100 text-green-800 py-3 rounded-xl font-bold hover:bg-green-200">Fermer</button></div></div>
      )}
    </div>
  );
};
